...
ts = tiledlayout(t1,'TileSpacing','tight','Padding','tight');
ts.Layout.Tile = 'south';
tn = tiledlayout(t1,'TileSpacing','tight','Padding','tight');
tn.Layout.Tile = 'north';
count = 1;
for j=1:3
for k=1:3
% --- ROW 1: Data Points ---
t2 = tiledlayout(t1,nplots,1,'TileSpacing','tight','Padding','tight');
t2.Layout.Tile = (j-1)*size(surf_images,1)+k;
ax = nexttile(t2,1);
...
if j==1
    title(t2,"Method "+string(k));
end
if k==1
    ylabel(t2,"Surface "+sNames{j,k})
end
