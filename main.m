...
% Perform pre-BO actions like table initialization, that vary by method

if method==1
    % IMPLEMENT JAFARI AND GANS
elseif method==2
    gprMdls=cell(m,1);
elseif method==3
    gprMdls=cell(m,1);
...
Train = table(zeros(m,1),zeros(m,1),zeros(m,1),zeros(m,1),zeros(m,1),'VariableNames',{'TotAE','MaxAE','MAE','RMSE','R2'});
Test = table(zeros(m,1),zeros(m,1),zeros(m,1),zeros(m,1),zeros(m,1),'VariableNames',{'TotAE','MaxAE','MAE','RMSE','R2'});
modelPerformance=table(Train, Test, zeros(m,1),'VariableNames',{'Train','Test','MaxCIWidth'});
...
elseif method==2 || method==3
    ...
    for iter=iter 
        ...
        modelPerformance.Train.R2(iter) = 1-sum(trainError.^2)/sum((zBl-mean(zBl)).^2);
        ...
        % Mean Surface
        surf(xBp,yBp,zBp,reshape(testabsError,res_s,res_s),'LineStyle','none');
        % Surface B
        ...
        % else
        %     imwrite(imind,cm,strcat(fname,'.gif'),'gif','WriteMode','append','DelayTime', 0.25);
        % end
    end
end
...
%% Plotting
% Quick Error Reshape
testError = reshape(testError,res_s,res_s);
testabsError = abs(testError);
...
hold on
% Mean Surface
surf(xBp,yBp,zBp,testabsError,'LineStyle','none');
% Surface B
% surf(x,y,zB,'FaceAlpha',.1,'FaceColor',[.5 0 .5],'EdgeColor',[.5 0 .5],'LineStyle','none');
% Maximum Surface
surf(xBp,yBp,zBpCImx, ...
...
% 
% % Show GIF
% [gifImage, cmap] = imread(strcat(fname,'.gif'), 'Frames', 'all');
% gifsize=size(gifImage);
% hi=implay(gifImage);
% hi.Visual.ColorMap.Map=cmap;
% hi.Parent.Position=[0,0,gifsize(2),gifsize(1)];