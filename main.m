...
% Initialize stopping criteria variables
ConfidenceTestingValues = [];
...
elseif method==2 || method==3
    ...
    for iter=iter 
        ...
        % Stop BO loop when stopping criteria is met
        if stoppingcriteria
            ConfidenceTestingValues = cat(1, ConfidenceTestingValues, modelPerformance(iter,:).CV.MAE);
            if size(ConfidenceTestingValues, 1) >= 3
                if ConfidenceTestingValues(end) < criticalNumber2 &&...
                   ConfidenceTestingValues(end - 1) < criticalNumber2 &&...
                   ConfidenceTestingValues(end - 2) < criticalNumber2
                    criticalPrintingValue = iter;
                    break
                end
            end
        end
